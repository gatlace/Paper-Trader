...
#define TA_CANDLERANGE(SET, IDX) \
   (TA_CANDLERANGETYPE(SET) == ENUM_VALUE(RangeType, TA_RangeType_RealBody, RealBody) ? TA_REALBODY(IDX) : (TA_CANDLERANGETYPE(SET) == ENUM_VALUE(RangeType, TA_RangeType_HighLow, HighLow) ? TA_HIGHLOWRANGE(IDX) : (TA_CANDLERANGETYPE(SET) == ENUM_VALUE(RangeType, TA_RangeType_Shadows, Shadows) ? TA_UPPERSHADOW(IDX) + TA_LOWERSHADOW(IDX) : 0)))
#define TA_CANDLEAVERAGE(SET, SUM, IDX) \
   (TA_CANDLEFACTOR(SET) * (TA_CANDLEAVGPERIOD(SET) != 0.0 ? SUM / TA_CANDLEAVGPERIOD(SET) : TA_CANDLERANGE(SET, IDX)) / (TA_CANDLERANGETYPE(SET) == ENUM_VALUE(RangeType, TA_RangeType_Shadows, Shadows) ? 2.0 : 1.0))
